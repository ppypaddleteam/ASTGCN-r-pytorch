...
from lib.utils import load_graphdata_channel1, get_adjacency_matrix, compute_val_loss_mstgcn, predict_and_save_results_mstgcn, RMSLELoss, predict_and_save_results_mstgcn_without_evaluation
...
def test_main(global_step, data_loader, data_target_tensor, _mean, _std, type):
    '''
    ...
    predict_and_save_results_mstgcn_without_evaluation(net, data_loader, data_target_tensor, global_step, _mean, _std, params_path, type)


if __name__ == "__main__":

    # FIXME: input_data 是最近10天的数据： 如何获取？
    # num_of_rounds: 拓展多少轮： 3 round  * 10 days/round = 30
    
    # 最好的步数
    best_step = 21
    params_filename = os.path.join(params_path, 'epoch_%s.params' % best_step)
    print('load weight from:', params_filename)

    net.load_state_dict(torch.load(params_filename))

    # 这个函数在utils里的
    predict_and_save_results_mstgcn_without_evaluation(net, input_data, best_step, params_path, num_of_rounds, batch_size, type):
